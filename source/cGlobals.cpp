...
cGlobals::cGlobals()
{
	...
	// AnClark MOD: Explicitly initialize some index arrays.
	//              Otherwise, program will crash when initializing cGlobals!
	memset(iSrc, 0, sizeof(int) * 32);
	memset(iDst, 0, sizeof(int) * 32);
};
cGlobals::~cGlobals()
{
	delete waveform;
