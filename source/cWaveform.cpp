...
cWaveform::cWaveform()
{
	...
	// AnClark MOD: Set wavetable data pointers to nullptr to prevent possible bugs
	sawTable = nullptr;
	squareTable = nullptr;
	...
	{
		// AnClark NOTICE:
		//         For RAND_MAX, different platforms has different behaviors.
		//         On Windows, RAND_MAX = 32767, while on Linux it's 2147483647.
		//         To prevent unexpected behaviors, I prefer a fixed value (the same as Windows).
#if 0	// Don't use RAND_MAX.
	    random = ((float)rand() / (float)(RAND_MAX + 1));
#else
		constexpr auto MY_RAND_MAX = 32767;
		random = ((float)rand() / (float)(MY_RAND_MAX + 1));
#endif
...
cWaveform::~cWaveform()
{
	// AnClark MOD: Remember to free waveform data
	delete sawTable;
	delete squareTable;
...
void cWaveform::readResources()
{
	...
	FreeResource(resDataLoad);
#else
	sawTable = new float[MirWavetableData::wavSawDataSize];
	memcpy (sawTable,((void*)MirWavetableData::wavSawData),MirWavetableData::wavSawDataSize);

	squareTable = new float[MirWavetableData::wavSquareDataSize];
	memcpy (squareTable,((void*)MirWavetableData::wavSquareData),MirWavetableData::wavSquareDataSize);

	triangleTable = new float[MirWavetableData::wavTriangleDataSize];
	memcpy (triangleTable,((void*)MirWavetableData::wavTriangleData),MirWavetableData::wavTriangleDataSize);
